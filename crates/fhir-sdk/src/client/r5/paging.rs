...
impl<R: NamedResource> Paged<R> {
	...
	pub(crate) fn new_typed(client: Client<FhirR5>, url: Url) -> Self {
		Self::new(client, url, Some(R::TYPE))
	}
}

impl Paged<Resource> {
	pub(crate) fn new_system(client: Client<FhirR5>, url: Url) -> Self {
		Self::new(client, url, None)
	}
}

impl<R> Paged<R> {
	fn new(client: Client<FhirR5>, url: Url, resource_type: Option<ResourceType>) -> Self {
